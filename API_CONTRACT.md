...
```
`role` is always `"doctor"` or `"patient"`.
...
```
`status` is `"pending"`, `"accepted"`, or `"rejected"`. `requested_by_id` is whichever of `patient_id`/`doctor_id` sent the request — compare it to your own id to know whether you're waiting on the other party or they're waiting on you. `responded_at` is `null` until the recipient accepts or rejects.
