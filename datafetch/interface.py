...
Updated with full patient demographics (age, sex) and lowercase conversion for sex.
"""
...
def create_patient_interface(supabase: Client):
    ...
    sex = input("Sex (male/female/other): ").strip().lower()
    if sex not in ['male', 'female', 'other']:
        print_error("Sex must be one of: male, female, other.")
        wait_for_enter()
        return
...
def login_interface(supabase: Client):
    ...
    try:
        # Include age and sex in the query
        result = supabase.table('patients') \
            .select('id, password_hash, name, consented_at, age, sex') \
            .eq('id', patient_id) \
            .execute()
        ...
        # Show demographics on login (capitalize for display)
        age = patient.get('age', '?')
        sex = patient.get('sex', '?').capitalize()
        print_success(f"Welcome, {patient['name']}! ({age} yrs, {sex})")
...
def show_history_interface(supabase: Client, patient_id: str, patient_name: str):
    ...
    try:
        # Fetch patient demographics (capitalize sex)
        patient_info = supabase.table('patients') \
            .select('age, sex') \
            .eq('id', patient_id) \
            .execute()
        if patient_info.data:
            info = patient_info.data[0]
            sex_display = info.get('sex', '?').capitalize()
            print(f"   👤 Age: {info.get('age', '?')} | Sex: {sex_display}\n")
